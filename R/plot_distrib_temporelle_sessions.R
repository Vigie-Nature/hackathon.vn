...
#' p <- plot_distrib_temporelle_sessions(df_vn = aspifaune, periode = "annee")
#' print(p)

plot_distrib_temporelle_sessions <- function(df_vn, periode="annee"){
  ...
  df_vn_session_unique <- unique(df_vn[, c("session_id", "site_id", "annee", "mois")])
  ...
  if(periode %in% "annee"){
    ...
    # Comptage
    nb_session_par_site <- df_vn_session_unique %>%
      group_by(site_id, annee) %>%  # Regroupe par annee et par site
      summarise(nombre_session = n()) %>%  # Compte le nombre de comptages
      arrange(site_id, annee)  # Trie les résultats par annee et site
    ...
    if(periode %in% "mois"){
      ...
      # Comptage
      nb_session_par_site <- df_vn_session_unique %>%
        group_by(site_id, mois) %>%  # Regroupe par année et par site
        summarise(nombre_session = n()) %>%  # Compte le nombre de comptages
        arrange(site_id, mois)  # Trie les résultats par annee et site
