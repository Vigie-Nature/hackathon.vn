...
makeSpeciesMap <- function(data, speciesList, interestVar, path){
  ...
  if("longitude" %in% colnames(data) & "latitude" %in% colnames(data)){
    ...
    listOfPlots = lapply(speciesList, function(sp){
      ...
      dataSp_Pres = data[data$taxon == sp & data[,interestVar[1]] > 0,]
      
      # Extract coordinates of sites where species is present
      dataSite_Pres = unique(dataSp_Pres[,c("site_id", "longitude", "latitude")])
      
      # Extract coordinates of all sampled sites
      dataSite_All = unique(data[,c("site_id", "longitude", "latitude")])
      
      # Attribute a present/absent value for each square
      dataSite_All$Type = "Absence"
      dataSite_All$Type[dataSite_All$site_id %in% dataSite_Pres$site_id] = "Présence"
